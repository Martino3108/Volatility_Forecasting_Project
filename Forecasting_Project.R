...
#remove stock 31
#trying different test stocks I noticed that the naive predictor obtains always an RMSPE between 0.2174 and 0.4519
...
#0.2644
#hypotesis are not verified for both models, I try remooving outliers
...
#from now on I will use "log" models without outliers (16361 data points out of 16650) train_data_new_log

#I try to build a fixed effect model on alpha
...
#I test if also random effects are significant
...
#now I try a six steps exponential weighted average
#I use cross validation in order to estalish the best parameter
sum<-0
count<-0
vect=numeric(10)
...
#Best model is log(target)~log(sigma_weighted_6)+log(kurtosis)+wap with stock based fixed effect
